...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        height: an integer > 0
        """
        if width > 0 and isinstance(width, int) and height > 0 and isinstance(height, int):
            self.width = width
            self.height = height
        else:
            raise ValueError("Height and weight must be an integer greater than 0")
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if not self.isTileCleaned(int(pos.x), int(pos.y)):
            self.board[int(pos.x), int(pos.y)] = 1
        return self.board
    ...
    def getRandomPosition(self):
        ...
        """
        x = int(math.floor(random.randrange(0, self.width)))
        y = int(math.floor(random.randrange(0, self.height)))
...
print(runSimulation(5, 3.0, 15, 13, .98, 30, StandardRobot))
...
#showPlot2("Standard vs RandomWalk Robots", "Height/Width Ratio", "Number of Steps")
# === Problem 6
